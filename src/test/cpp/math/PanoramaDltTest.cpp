...
namespace math
{
	namespace test
	{
		...
		TEST_F( PanoramaDltTest, DISABLED_SimpleTransform )
		{
			...
			ASSERT_TRUE( ( p0i1 ).isApprox( transfP0, 1.0e-2 ) );
			ASSERT_TRUE( ( p1i1 ).isApprox( transfP1, 1.0e-2 ) );
			ASSERT_TRUE( ( p2i1 ).isApprox( transfP2, 1.0e-2 ) );
			ASSERT_TRUE( ( p3i1 ).isApprox( transfP3, 1.0e-2 ) );
		}
	}
}