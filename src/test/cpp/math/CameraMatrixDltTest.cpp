	...
	namespace test
	{
		...
		TEST_F( CameraMatrixDltTest, DISABLED_RandomPoints )
		{
			...
			for( int i = 0; i < numPoints; ++i )
			{
				...
				VectorXd p0 = m_K0.inverse() * m_P0 * point3D; p0 = p0 / p0[ 2 ];
				VectorXd p1 = m_K1.inverse() * m_P1 * point3D; p1 = p1 / p1[ 2 ];
			...
			resultP1.block( 0, 0, 3, 3 ) = m_K1 * resultP1.block( 0, 0, 3, 3 );
			
			MatrixXd resultP0 = dlt.getP0();
			resultP0.block( 0, 0, 3, 3 ) = m_K0 * resultP0.block( 0, 0, 3, 3 );
		...
		TEST_F( CameraMatrixDltTest, DISABLED_LuizsDataset )
		{
		...
		TEST_F( CameraMatrixDltTest, DISABLED_RealDataset )
		{
			int numTotalImgs = 10;
